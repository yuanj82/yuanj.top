...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193614.png)

我们现在可以看到下方出现一个列表，点击 RAP ID 列的 Os05g0409300 进入该基因的详情页

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193633.png)

再点击 Transcript variants （转录变体）下的基因名称进入该基因转录本的详情页

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193655.png)

下滑到最下方即可得到蛋白质序列，复制下来保存即可

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193722.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193755.png)

搜索完成后我们可以看到，SMART 网站已经将结构域的位置等信息列出来了，下方表格中 E-value
 小于十的负五次方的通常认为其可信，而 E-value 值为 N/A 的即为空值，不需要管它

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193820.png)

点击下方表格中的结构域名称我们可以得到该结构域的全称、描述等详细信息，可以看到我这个基因的结构域为 Cystatin，所以后面我便以水稻 Cystatin 基因家族为基础展开一系列生物信息学分析

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601193831.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601194409.png)

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601194438.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601194651.png)

再点击 Alignments——Fomat 选择 stockholm-gengerate 下载 Stockholm file，此处下载的文件名为 PF00031_seed.txt

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601194914.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195022.png)

在 Windows 资源管理器地址栏中输入 CMD 回车在此位置打开 CMD 命令行，命令与 Linux 系统下一致

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195106.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195200.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195330.png)

得出结果后，查看 evalue 这一列，所有的值均小于 10 的-5 次方，我们现在可以认为 HMMER 搜索结果是准确的，这 14 个基因都属于 Cystain 基因家族

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195426.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195521.png)

然后使用 TBtools 进行可视化

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195739.png)

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195751.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195838.png)

然后使用 TBtools 进行可视化

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195927.png)

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601195942.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200515.png)

比对完成后点击左上角的 Data，选择 Phylogenetic Analysis 进行进化树分析

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200615.png)

进化树分析完成后，点击上方 Phylogeny，选择 NJ 算法构建进化树

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200701.png)

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200724.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200737.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200843.png)

得到分析结果

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601200902.png)

### 染色体定位分析

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201108.png)

得到染色体位置分布图

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201134.png)

我们导入重命名文件，使其名称全部变为我们命名的格式

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201151.png)

注意重命名文件的格式

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201453.png)

可以在这里对颜色和字体进行简单的美化

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201539.png)

当然，如果你不想对它们进行统一的更改，可以点击任意一个基因名称，再点击右键对其进行单独的更改，对于染色体的名称也同样适用

调整过后的图就好看了很多

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201554.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201651.png)

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201710.png)

### 基因表达量分析

首先在 [Rice Expression Database](http://expression.ic4r.org) 中搜索基因列表

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201730.png)

选择 DRP000391 这个项目

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201742.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601201850.png)

整理成如下样式

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202026.png)

然后对数据进行 LOG2 处理，如果出现#NUM! 字样，全部改为 0

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202044.png)

重新整理得到最终数据，打开 TBtools 的 Heat Map 功能，把刚刚整理的数据粘贴进去进行绘图

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202109.png)

得到基因家族的表达热图

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202126.png)

对基因进行重命名即将复制进去的表格中的基因名称进行修改即可

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202144.png)

然后重新制图

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202157.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202448.png)

搜索完成后就得到了蛋白质的互相作用网络

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202528.png)

点击每一个圆珠我们就可以查看其信息，包括基因 ID、物种等，圆珠直接的黑色线条表示共表达蛋白质，黄色线条表示在其他生物体中共同提及推定的同源物。

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202544.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202619.png)

### 多序列比对

在 [CLUSTALW 网站](https://www.genome.jp/tools-bin/clustalw) 上传基因家族的蛋白质序列进行多序列比对，比对完成后下载比对结果的。aln 文件

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202649.png)

打开 [ESPript](https://espript.ibcp.fr/ESPript/cgi-bin/ESPript.cgi) ，上传刚刚的。aln 文件进行可视化

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202703.png)

下载结果中的 pdf 文件打开后即可得到多序列比对的图像

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202715.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202742.png)

使用方法和之前的插件一样，这个插件跑一次整个水稻的共线性分析只需要几分钟

最终我绘制的图像是这样的

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202849.png)

我们还是希望使用自己命名的基因名称，对显示 ID 文件中的信息进行修改，使用记事本的查找替换功能将文件中转录本的 ID 全部替换为重命名的名称即可

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202905.png)

我们再对其进行一些美化，具体操作请参照 [共线性分析美化](https://www.bilibili.com/video/BV1js4y197d7/?vd_source=f785a7035a51b96b9abcf6b14d1036ea)

下图是我最终美化好的

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202915.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601202947.png)

**进化树+Motif+Domain**

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203017.png)

**进化树+Motif+Domain+显示外显子、内含子**

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203035.png)

**进化树+Motif+Domain+显示外显子、内含子+顺式作用元件**

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203136.png)

最终得到一张信息整合的图

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203149.png)
...
![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203418.png)

如果想对基因进行重命名，准备好重命名文件，文件格式与染色体定位分析时的一样

在下列位置导入重命名文件

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203551.png)

另外，如果我们不要进化树，比如在基因很少或只有单条基因的时候，可能无法做出进化树或者进化树很稀疏，那么我们可以不用导入进化树，在 Gene Structure View 功能填写进化树信息的地方我们填入基因列表即可

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203616.png)

得出图像

![](https://gcore.jsdelivr.net/gh/yuanj82/static/blog/20230601203633.png)
